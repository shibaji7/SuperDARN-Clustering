...
"""plot_utils.py: module is dedicated to plots of the images."""
...
def getD2N(dates):
    """ Returns date to numbs"""
    return [date2num(d) for d in dates]
...
class RangeTimePlot(object):
    ...
    def addClusterPlot(self, data_dict, clust_flg, beam, title, show_closerange=True, xlabel=""):
        ...
        gate = np.hstack(data_dict["slist"])
        allbeam = np.hstack(data_dict["bmnum"])
